...
#ifndef HX_KAIROS_ABSOREL_HPP
#define HX_KAIROS_ABSOREL_HPP
...
struct Absorel
{
	...
	Absorel operator+(const Absorel& offset);
	template <typename T>
	Absorel operator-(const T& offset);
	Absorel operator-(const Absorel& offset);
	template <typename T>
	...
	bool operator<(const Absorel& position);
	bool operator>(const Absorel& position);
	friend std::ostream& operator<<(std::ostream& out, const Absorel& position);

private:
	template <typename T>
	Absorel positionFromNumber(const T& n); // return position created from any number type
};
...
template <typename T>
Absorel Absorel::positionFromNumber(const T& number)
{
