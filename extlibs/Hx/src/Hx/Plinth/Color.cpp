...
Rgb rgbFromCmh(double c, double m, double h)
{
	...
	double x{ c * (1.0 - hx::mod(static_cast<double>(abs(h * 6.0)), 2.0) - 1.0) };
...
Rgb rgbFromValue(long int value)
{
	...
	int red{ static_cast<int>(value / (base * base)) };
	value -= red * base * base;
	int green{ static_cast<int>(value / base) };
	value -= green * base;
	int blue{ static_cast<int>(value) };
